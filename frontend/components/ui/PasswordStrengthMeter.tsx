...
export function PasswordStrengthMeter({ password }: { password: string }) {
  ...
  return (
    <div className="rounded-xl border border-[rgba(255,140,66,0.12)] bg-[#fff8f0]/25 px-2.5 py-1.5">
      ...
      <div className="mt-1.5 h-1.5 overflow-hidden rounded-full bg-[#f4e7dc]">
      ...
      <div className="mt-1.5 flex flex-wrap items-center gap-x-2 gap-y-0.5">
        {strength.rules.map((rule) => (
          <span key={rule.label} className={classNames("inline-flex min-w-0 items-center gap-1 text-[10px] font-bold leading-3", rule.met ? tone.rule : "text-slate-400")}>
            <span
            ...
            >
              {rule.met ? <CheckIcon /> : <span className="h-1 w-1 rounded-full bg-slate-300/80" />}
            </span>
...
function CheckIcon() {
  return (
    <svg viewBox="0 0 16 16" aria-hidden="true" className="h-2 w-2" fill="none" stroke="currentColor" strokeWidth="2.4">
